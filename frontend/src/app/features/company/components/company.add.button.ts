...
@Component({
  ...
  template: `
    ...
    <p-dialog [(visible)]="dialogVisible" [modal]="true">
      <ng-template #header> Unternehmen erstellen </ng-template>
      <form [formGroup]="formGroup()" (submit)="submitForm()">
        ...
        <div class="flex flex-col gap-0.5">
          @if (httpError()) {
            <span
              >Ein Fehler ist bei der Übertragung der Daten aufgetreten. Bitte versuche es
              erneut!</span
            >
          }
  ...
  readonly formGroup = signal(
    new FormGroup({
      company: new FormControl<string | null>(null, {
        ...
        asyncValidators: [this.companyNameValidator.validate.bind(this.companyNameValidator)],
        updateOn: 'change',
      }),
    }),
  );
