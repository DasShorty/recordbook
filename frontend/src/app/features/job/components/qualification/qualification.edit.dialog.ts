    ...
    <p-dialog [(visible)]="dialogVisible" [modal]="true">
      ...
      <form [formGroup]="formGroup" (submit)="submitForm()">
        ...
        <div class="flex items-center gap-4 mb-4">
          ...
          <p-inputNumber formControlName="minimumDuration" id="minimumDuration" [step]="0.1" suffix=" h" mode="decimal"
                         class="flex-auto" [showButtons]="true"></p-inputNumber>
        </div>
        <div class="flex flex-col gap-0.5">
...
export class QualificationEditDialog {
  ...
  protected submitForm() {
    ...
    if (formContent.name == null || formContent.minimumDuration == null || formContent.description == null) {
      return;
    }

    if (this.editMode()) {
      this.qualificationStore.updateQualification(formContent as Qualification)
        .then(res => this.handleResponse(res));
      return;
    }

    this.qualificationStore.createQualification({
  ...
  private handleResponse(optional: Optional<Qualification>) {
    ...
    if (this.editMode()) {
      ...
      this.qualificationStore.retrieveQualifications(this.qualificationStore.limit(), this.qualificationStore.offset());
    }
