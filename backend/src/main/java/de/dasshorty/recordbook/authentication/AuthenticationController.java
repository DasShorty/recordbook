...
public class AuthenticationController {
    ...
    private void setTokenCookies(HttpServletResponse response, String accessToken, long accessTime, String refreshToken, long refreshTime) {

        ResponseCookie accessCookie = this.createResponseCookie("access_token", accessToken, Duration.ofSeconds(accessTime));
        ResponseCookie refreshCookie = this.createResponseCookie("refresh_token", refreshToken, Duration.ofSeconds(refreshTime));
    ...
    private ResponseCookie createResponseCookie(String name, String value, Duration duration) {
        ...
        return ResponseCookie.from(name, value).httpOnly(true).secure(this.secureHttpCookies).sameSite(SAME_SITE).domain(this.cookieDomain)
